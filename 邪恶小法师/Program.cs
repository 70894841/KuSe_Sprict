...
namespace VeigarEndboss
{
    class Program
    {
        ...
        {
            // Create menu
            menu = new Menu("閭伓灏忔硶甯堚憿鈶犫懅鈶ㄢ憽鈶モ憿", "hells" + champName, true);
            ...
            // Orbwalker
            //Menu orbwalker = new Menu("琛ュ垁璧扮爫", "orbwalker");
            //OW = new LXOrbwalker(orbwalker);
            ...
            combo.AddItem(new MenuItem("comboUseQ", "浣跨敤 Q").SetValue(true));
            combo.AddItem(new MenuItem("comboUseW", "浣跨敤 W").SetValue(true));
            combo.AddItem(new MenuItem("comboUseE", "浣跨敤 E").SetValue(true));
            combo.AddItem(new MenuItem("comboUseR", "浣跨敤 R").SetValue(true));
            combo.AddItem(new MenuItem("comboUseIgnite", "浣跨敤鐐圭噧").SetValue(true));
            combo.AddItem(new MenuItem("comboActive", "杩炴嫑鎸夐敭").SetValue(new KeyBind(32, KeyBindType.Press)));
            menu.AddSubMenu(combo);

            // Harass
            Menu harass = new Menu("娑堣€楅€夐」", "harass");
            harass.AddItem(new MenuItem("harassUseQ", "浣跨敤 Q").SetValue(true));
            harass.AddItem(new MenuItem("harassUseW", "浣跨敤 W").SetValue(true));
            harass.AddItem(new MenuItem("harassMana", "娉曞姏鍊肩櫨鍒嗙偣(%)").SetValue(new Slider(30)));
            harass.AddItem(new MenuItem("harassActive", "娑堣€楀揩鎹烽敭").SetValue(new KeyBind('C', KeyBindType.Press)));
            menu.AddSubMenu(harass);

            // WaveClear
            Menu waveClear = new Menu("娓呯嚎", "waveClear");
            waveClear.AddItem(new MenuItem("waveUseW", "浣跨敤 W").SetValue(true));
            waveClear.AddItem(new MenuItem("waveNumW", "W鍙懡涓皬鍏垫暟").SetValue(new Slider(3, 1, 10)));
            waveClear.AddItem(new MenuItem("waveActive", "娓呯嚎鎸夐敭").SetValue(new KeyBind('V', KeyBindType.Press)));
            menu.AddSubMenu(waveClear);

            // Misc
            Menu misc = new Menu("鏉傞」", "misc");
            misc.AddItem(new MenuItem("miscStackQ", "鑷姩鍙犲姞Q").SetValue(true));
            misc.AddItem(new MenuItem("miscAutoW", "W鑷姩涓婂墠鍑绘檿").SetValue(true));
            menu.AddSubMenu(misc);

            // Drawings
            Menu drawings = new Menu("缁樺埗鑼冨洿", "drawings");
            drawings.AddItem(new MenuItem("drawRangeQ", "Q 鑼冨洿").SetValue(new Circle(true, Color.FromArgb(150, Color.IndianRed))));
            drawings.AddItem(new MenuItem("drawRangeW", "W 鑼冨洿").SetValue(new Circle(true, Color.FromArgb(150, Color.MediumPurple))));
            drawings.AddItem(new MenuItem("drawRangeE", "E 鑼冨洿").SetValue(new Circle(false, Color.FromArgb(150, Color.DarkRed))));
            drawings.AddItem(new MenuItem("drawRangeR", "R 鑼冨洿").SetValue(new Circle(false, Color.FromArgb(150, Color.Red))));
            menu.AddSubMenu(drawings);

            // Finalize menu
            menu.AddToMainMenu();
            PrintMessage("閭伓灏忔硶甯堝凡鍔犺浇");
        }
        public static void PrintMessage(string msg) // Credits to ChewyMoon, and his Brain.exe
        {
            Game.PrintChat("<font color=\"#FFD700\"><b>鑻︽订鍝モ憿鈶犫懅鈶ㄢ憽鈶モ憿:</b></font> <font color=\"#FF0033\">" + msg + "</font>");
        }
    }
}
